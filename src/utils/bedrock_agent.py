...
class Guardrail:
    def __init__(
        ...
        if verbose:
            logger.info(f"Found {len(resp['guardrails'])} guardrails: {resp['guardrails']}")
            logger.info(f"Looking for guardrail: {self.name}")
        for _gr in resp["guardrails"]:
            if _gr["name"] == self.name:
                if verbose:
                    logger.info(f"Found guardrail: {_gr['id']}")
        ...
        if verbose:
            logger.info(f"Guardrail created: {resp}")
...
class Agent:
    ...
    def __init__(
        ...
        if not Agent.default_force_recreate:
            ...
            if tools is None and self.tool_code is not None and self.tool_code != "ROC":
                logger.info(f"Adding action group with Lambda: {self.tool_code}...")
                ...
                logger.info(f"Adding action group with Return of Control...")
                ...
                _tool_num = 1
                for _tool in tools:
                    logger.info(f"Adding tool: {_tool['definition']['name']}...")
                    # logger.info(f"Adding action group for tool: {str(_tool.definition['name'])}...")
    ...
    def prepare(self, alias="prod"):
        ...
        if self.needs_preparation():
            ...
            logger.info("Agent already prepared")
    ...
    def attach_tool(self, tool: Tool) -> None:
        ...
        if Agent.NO_TOOL_USE_INSTRUCTION in instructions:
            logger.info(f"Replacing instructions to not use tools...")
        ...
        if self.has_action_group(tool.name):
            logger.info(f"Action group {tool.name} already exists, skipping...")
            return
...
class SupervisorAgent:
    def __init__(
        ...
        if verbose:
            logger.info(f"Collaborator agents: {self.collaborator_agents}")
            logger.info(f"Collaboration type: {self.collaboration_type}")
            logger.info(f"Instructions: {self.instructions}")
            logger.info(f"Routing classifier model: {self.routing_classifier_model}")
            logger.info(f"Collaborator objects: {self.collaborator_objects}")
            logger.info(f"Tool code: {self.tool_code}")
            logger.info(f"Tool defs: {self.tool_defs}")
        ...
        if not Agent.default_force_recreate:
            ...
            try:
                if verbose:
                    logger.info(f"Checking if supervisor agent exists: {self.name}...")
                ...
                if verbose:
                    logger.info(f"multi_agent_names: {self.multi_agent_names}")
            except Exception as e:
                logger.error(f"Error finding existing supervisor agent: {e}")
                raise
            return
        ...
        _collab_list = []
        if verbose:
            logger.info(f"  Supervisor '{self.name}' is adding the following collaborators:")
        ...
        if verbose:
            logger.info(f"routing model: {self.routing_classifier_model}")
        ...
        logger.info(f"  associating sub-agents / collaborators to supervisor...")
        ...
        # w/o sub-agents
        if kb_id is not None:
            logger.info(f"Associating KB: {kb_id} to supervisor...")
            agents_helper.wait_agent_status_update(
                self.supervisor_agent_id
            )  # wait to be out of "Versioning" state
            agents_helper.associate_kb_with_agent(
        ...
        if verbose:
            logger.info(f"  multi-agent names: {self.multi_agent_names}")
    ...
    def _get_collab_alias_arn(self, collab_name):
        # logger.info(f"Finding argn for collab: {collab_name}")
        for _collab_obj in self.collaborator_objects:
            # logger.info(f"  comparing with name: {_collab_obj.name}")
            if _collab_obj.name == collab_name:
                # logger.info(f"  found name, returning arn: {_collab_obj.agent_alias_arn}")
                return _collab_obj.agent_alias_arn
        # logger.info(f"  did not find name, returning None")
        return None
...
be sure to include the details as input to the task.\n\n"""
        ...
        if verbose and enable_trace and trace_level == "core":
            logger.info(f"Here is the prompt being sent to the supervisor:\n{prompt}\n")
