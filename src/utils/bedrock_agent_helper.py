...
class AgentsForAmazonBedrock:
    ...
    def __init__(self):
        ...
        self._boto_session = boto3.Session(profile_name=profile_name)
        self._region = self._boto_session.region_name
        self._account_id = self._boto_session.client("sts").get_caller_identity()["Account"]

        self._bedrock_agent_client = self._boto_session.client("bedrock-agent")

        long_invoke_time_config = Config(read_timeout=600)
        self._bedrock_agent_runtime_client = self._boto_session.client(
        ...
        self._lambda_client = self._boto_session.client("lambda")
        self._s3_client = self._boto_session.client("s3", region_name=self._region)
        self._dynamodb_client = self._boto_session.client("dynamodb", region_name=self._region)
        self._dynamodb_resource = self._boto_session.resource("dynamodb", region_name=self._region)
    ...
    def get_agent_latest_alias_id(self, agent_id: str, verbose: bool = False) -> str:
        ...
        for _summary in _agent_aliases["agentAliasSummaries"]:
            ...
            if _curr_update > _latest_update:
                ...
                # logger.info(f"agent id: {agent_id}, routing config: {_summary['routingConfiguration']}")
                # _alias_version = _summary['routingConfiguration'][0]['agentVersion']

        if verbose:
            logger.info(f"for id: {agent_id}, picked latest alias: {_latest_alias_id}")
            logger.info(f"  updated at: {_latest_update}")
            logger.info(f"  alias name: {_alias_name}\n")  # , version: {_alias_version}\n")
    ...
    def get_agent_alias_arn(
        ...
        try:
            ...
            logger.error(f"Error getting agent alias: {e}")
    ...
    def delete_agent(
        ...
        if _target_agent is not None:
            ...
            if verbose:
                logger.info(f"Deleting aliases for agent {_agent_id}...")
            ...
            try:
                ...
                for alias in _agent_aliases["agentAliasSummaries"]:
                    ...
                    logger.info(f"Deleting alias {alias_id} from agent {_agent_id}")
        ...
        if _target_agent is not None:
            ...
            if verbose:
                logger.info(f"Deleting agent: {_agent_id}...")
            time.sleep(5)
        ...
        if delete_role_flag:
            ...
            if verbose:
                logger.info(f"Deleting IAM role: {_agent_role_name}...")
    ...
    def _create_agent_role(
        ...
        if verbose:
            logger.info(f"Creating IAM role for agent: {agent_name}")
        ...
        if reuse_default:
            ...
            try:
                ...
                if verbose:
                    ...
                    logger.info(f"using assume role json: {_assume_role_policy_document}")
            else:
                self._iam_client.put_role_policy(
    ...
    def wait_agent_status_update(self, agent_id):
        ...
        while agent_status.endswith("ING"):
            logger.info(f"Waiting for agent status to change. Current status {agent_status}")
            time.sleep(5)
        ...
        if _waited_at_least_once:
            logger.info(f"Agent id {agent_id} current status: {agent_status}")
    ...
    def create_agent(
        ...
        if verbose:
            logger.info(f"Created agent IAM role: {_role_arn}...")
            logger.info(f"Creating agent: {agent_name} with model: {_model_id}...")
        ...
        while not _agent_created and _num_tries <= 2:
            try:
                ...
                if verbose:
                    logger.info(f"Created agent, resulting id: {_agent_id}")
                    _get_resp = self._bedrock_agent_client.get_agent(agentId=_agent_id)
                    logger.info(_get_resp)
                _agent_created = True
                ...
                if _num_tries <= 2:
                    ...
                    if verbose:
                        logger.info(f"Giving up on agent creation after 2 tries.")
                    raise e
    ...
    def add_code_interpreter(self, agent_name: str) -> None:
        ...
        if _agent_action_group_resp["ResponseMetadata"]["HTTPStatusCode"] == 200:
            ...
            logger.error(f"Error adding code interpreter to agent: {_agent_action_group_resp}")
        return
    ...
    def add_action_group_with_lambda(
        ...
        if verbose:
            logger.info(f"Creating action group: {agent_action_group_name}...")
            logger.info(f"Lambda ARN: {_lambda_arn}")
            logger.info(f"Agent functions: {agent_functions}")
    ...
    def _make_fully_cited_answer(
        ...
        if enable_trace and trace_level == "all":
            logger.info(colored(f"cleaned text: '{_cleaned_text}'", "red"))
            logger.info(colored(f"original answer: '{orig_agent_answer}'", "red"))
        ...
        for _citation in _citations:
            if enable_trace and trace_level == "all":
                logger.info(colored(f"citation_idx: {_citation_idx}", "red"))
                logger.info(colored(f"full citation [{_citation_idx}]: {_citation}", "red"))
            ...
            if len(_refs) > 0:
                ...
                _ref_url = ""
                logger.info(colored(f"  !!no retrieved references for citation!!", "red"))

            _interim_answer = (
                _answer_prefix + _cleaned_text[_start:_end] + " [" + _ref_url + "] "
            )
            if enable_trace and trace_level == "all":
                logger.info(colored(f"fully cited: '{_interim_answer}'", "red"))
            ...
            if enable_trace and trace_level == "all":
                ...
                logger.info(f"citation span... start: {_start}, end: {_end}")
                logger.info(
                    f"citation based on span:====\n{_cleaned_text[_start:_end]}\n===="
                )
                logger.info(f"citation url: {_ref_url}\n============")
        ...
        if enable_trace and trace_level == "all":
            logger.info(colored(f"FINAL updated fully cited: {_fully_cited_answer}", "red"))
    ...
    def invoke_inline_agent(
        ...
        if enable_trace:
            if trace_level == "all":
                logger.info(f"invokeAgent API response object: {_agent_resp}")
            else:
                logger.info(
                    f"invokeAgent API request ID: {_agent_resp['ResponseMetadata']['RequestId']}"
                )
                logger.info(f"invokeAgent API session ID: {session_id}")
        ...
        try:
            ...
            for _event in _event_stream:
                ...
                if "chunk" in _event:
                    ...
                    if enable_trace and trace_level == "all":
                        # logger.info(f"tmp answer: '{_tmp_agent_answer}', streaming: {stream_final_response}, trace: {enable_trace}")
                        logger.info(
                    ...
                    #         logger.info(colored(f"Time to first token: {_time_to_first_token.total_seconds():,.1f}s\n", "yellow"))
                    # _num_response_chunks += 1

                    # if enable_trace and stream_final_response and _num_response_chunks < 3:
                    #     logger.info(colored(f"Answer chunk [{_num_response_chunks}]: {_tmp_agent_answer}", "blue"))
                    ...
                    if "attribution" in _event["chunk"]:
                        if "citations" in _event["chunk"]["attribution"]:
                            ...
                            if enable_trace and trace_level == "all":
                                logger.info(colored(f"Citations: {_citations}", "blue"))
                ...
                if "trace" in _event and enable_trace:
                    ...
                    if "routingClassifierTrace" in _event["trace"]["trace"]:
                        ...
                        if "modelInvocationInput" in _route:
                            _orch_step += 1
                            logger.info(colored(f"---- Step {_orch_step} ----", "green"))
                    ...
                    if "orchestrationTrace" in _event["trace"]["trace"]:
                        ...
                        if trace_level in ["core", "outline"]:
                            if "rationale" in _orch:
                                ...
                                logger.info(colored(f"{_rationale['text']}", "blue"))
                            ...
                            if "invocationInput" in _orch:
                                ...
                                if "actionGroupInvocationInput" in _input:
                                    ...
                                    if trace_level == "outline":
                                        ...
                                        logger.info(colored(f"  {_kb_query}\n", "magenta"))
                        ...
                        if "modelInvocationOutput" in _post:
                            ...
                            _total_llm_calls += 1
                            logger.info(colored("Agent post-processing complete.", "yellow"))
                            logger.info(
                                colored(
                                    f"Used LLM tokens, in: {_in_tokens}, out: {_out_tokens}",
                                    "yellow",
                    ...
                    if trace_level == "all":
                        logger.info(json.dumps(_event["trace"], indent=2))

                if "files" in _event.keys() and enable_trace:
                    console = Console()
                    files_event = _event["files"]
                    console.logger.info(Markdown("**Files**"))

                    files_list = files_event["files"]
                    for this_file in files_list:
                        logger.info(f"{this_file['name']} ({this_file['type']})")
            ...
            if enable_trace:
                ...
                if trace_level == "all":
                    logger.info(f"Returning agent answer as: {_agent_answer}")
            ...
            logger.error(f"Caught exception while processing input to invokeAgent:\n")
            logger.error(f"  for input text:\n{request_params['inputText']}\n")
            logger.error(
    ...
    def invoke(
        ...
        if enable_trace:
            if trace_level == "all":
                ...
                logger.info(f"invokeAgent API session ID: {session_id}")
                logger.info(f"  agent id: {agent_id}, agent alias id: {agent_alias_id}")
        ...
        try:
            ...
            logger.error(f"Caught exception while processing input to invokeAgent:\n")
            logger.error(f"  for input text:\n{input_text}\n")
            logger.error(f"  on agent: {agent_id}, alias: {agent_alias_id}")
            logger.error(
                f"  request ID: {_agent_resp['ResponseMetadata']['RequestId']}, retries: {_agent_resp['ResponseMetadata']['RetryAttempts']}\n"
            )
            logger.error(f"Error: {e}")
    ...
    def invoke_roc(
        ...
        # logger.info(pprint.plogger.info(agentResponse))
    ...
    def create_dynamodb(self, table_name, pk_item, sk_item):
        try:
            ...
            logger.info(f"Table {table_name} already exists, skipping table creation step")
    ...
    def load_dynamodb(self, table_name: str, items: List):
        try:
            ...
            logger.error(f"Error on loading process for table: {table_name}.")
    ...
    def query_dynamodb(
        ...
        try:
            ...
            logger.error(f"Error querying table: {table_name}.")
